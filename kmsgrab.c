...
static void *map_dumb(int fd, int handle, size_t size) {
	...
	ptr = mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, arg.offset);
	if (ptr == MAP_FAILED) {
...
//todo 
//    #if 1
//    kmssink(fd, dmafd, fb->bpp, fb->width, fb->height, fb->pitch);
//    #else
	fprintf(stderr, "fb=%p id=%d fd=%d handle=%d size=%dx%d(%d) bpp=%d(%d)\n",
		fb, fb->fb_id, dmafd, fb->handle, fb->width, fb->height,
		fb->pitch, fb->bpp, fb->depth);

	ptr = map_dumb(fd, fb->handle, size);
	if (ptr) {
...
static void usage(const char *prog, int fd) {
	...
	fprintf(stderr, "%s [--crtc|--connector|--plane|--fb] OBJ_ID\n", prog);
